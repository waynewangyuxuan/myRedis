...
static void handle_read(Conn* conn){
    ...
    if (conn->outgoing.size()){
        ...
        //this socket is likely to be writable now in a request-response model
        //we we let it write without waiting for the next iteration.
        return handle_write(conn);
    } //else, we need to wait for more data. so we still wnat to read.
}
